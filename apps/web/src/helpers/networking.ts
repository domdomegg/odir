...
const logoutOnTokenExpiry = (err: unknown) => {
  if (typeof err === 'object' && err !== null && 'isAxiosError' in err) {
    ...
    if (axiosError.response?.status === 401 && typeof axiosError.response.data === 'object' && axiosError.response.data !== null && 'message' in axiosError.response.data && typeof axiosError.response?.data?.message === 'string' && axiosError.response.data.message.startsWith('Token expired')) {
      setAuthState();
