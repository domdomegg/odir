# 🔃 Org Directory Server

Back-end code and resources for the Org Directory platform.
...
- `src`: The bulk of the source code - you probably want to work in here most of the time
  - `api`: Code for endpoints on the HTTP API. The file system structure represents the path and method used for the endpoint, e.g. `src/api/admin/teams/get.ts` represents the handler when for the route `GET /admin/teams`. See the request handling section for more details about writing API endpoints.
